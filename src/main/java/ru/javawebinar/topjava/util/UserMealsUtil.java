...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> map = new HashMap<>(); //Map for computing calories by the day
        List<UserMeal> filteredUserMealByTime = new ArrayList<>();

        for (UserMeal userMeal : meals) {
            if (userMeal.getDateTime().toLocalTime().compareTo(startTime) >= 0 && userMeal.getDateTime().toLocalTime().compareTo(endTime) <= 0)
                filteredUserMealByTime.add(userMeal);
            map.merge(userMeal.getDateTime().toLocalDate(), userMeal.getCalories(), Integer::sum);
        }
        List<UserMealWithExcess> userMealWithExcesses = new ArrayList<>();
        for (UserMeal userMeal : filteredUserMealByTime) {
            boolean excess = false;
            if (map.get(userMeal.getDateTime().toLocalDate()) > caloriesPerDay) excess = true;
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        meals.forEach(meal -> {
            ...
            if (meal.getDateTime().toLocalTime().compareTo(startTime) >= 0 && meal.getDateTime().toLocalTime().compareTo(endTime) <= 0)
                filteredUserMealByTime.add(meal);
        });
        List<UserMealWithExcess> userMealWithExcesses = new ArrayList<>();
        filteredUserMealByTime.forEach(meal -> {
            boolean excess = false;
            if (map.get(meal.getDateTime().toLocalDate()) > caloriesPerDay) excess = true;
